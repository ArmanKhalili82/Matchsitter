...
const buttonVariants = cva(['h-14 group relative w-auto flex items-center justify-center !rounded-100 bg-primary text-primary-foreground'], {
  variants: {
    ...
    variant: {
      ...
      outline: [
        ...
        'disabled:ring-border-300 disabled:text-textBody-100',
      ],
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ children, className, loading, variant, size, asChild = false, ...props }, ref) => {
    ...
    return (
      <Comp className={cn(buttonVariants({ variant, size, className: cn({ '[&>*]:invisible loading': loading }, className) }))} ref={ref} {...props} >
        <div className='contents'>{children}</div>
        <div className='absolute group-[.loading]:!visible invisible inset-0  flex  items-center justify-center'>
          Loading
        </div>
      </Comp>
    )
